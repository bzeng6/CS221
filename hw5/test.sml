structure Test = struct
  ...
  fun recordcheck () =
    let
      val _ = Check.expect (RecordCheck.check(L.Record([("label1", L.True), ("label2", L.Record([("label1", L.Int(8)), ("label2", L.Unit)])), ("label3", L.False)])),
                            L.Record([("label1", L.True), ("label2", L.Record([("label1", L.Int(8)), ("label2", L.Unit)])), ("label3", L.False)]),
                            "record1")
      val _ = Check.expect (RecordCheck.check(L.App(L.True, L.False)), L.App(L.True, L.False), "record2")
      val _ = Check.expect (RecordCheck.check(L.Record([("label1", L.Int(88))])), L.Record([("label1", L.Int(88))]), "record3")                  
      val _ = Check.exn (fn () => RecordCheck.check(L.Record([])), "badScan1")
      val _ = Check.exn (fn () => RecordCheck.check(L.Record([("label1", L.True), ("label1", L.True)])), "badScan2")                      
      (* val _ = RecordCheck.check(Parse.parse(Scan.scan "[lam x (~a I ~b B ~c U) 0]")); *)
    in
      TextIO.print "recordcheck tests done\n"
    end
  ...
  fun subty () =
    let
      ...
      val _ = chkT "(~a (~a I ~b B) ~b B)" "(~a (~a I) ~b B)"
      val _ = chkT "(~a (~a I ~b B) ~b B ~c B)" "(~a (~a I ~b B) ~b B)"
...
(*      val _ = chkT "(~a (~x I ~y I) ~b B ~c I)" "(~b B ~a (~x I))" (* all subtyping ex from ed *) *)
      val _ = chkF "(~a I)" "(~a I ~b B)"
      (* val _ = chkT "(~x (~a I ~b I) ~y I)" "(~x (~a I))" *) (* #262 *)
    in
